...
impl ShareContract {
    pub fn deploy() -> Self {
        // Create admin.
        let tmp3 = SecretKey::ed25519_from_bytes([1u8; 32]).unwrap();
        let admin_key = PublicKey::from(&tmp3);
        let admin_addr = AccountHash::from(&admin_key);

        // Create plain user.
        let tmp4 = SecretKey::ed25519_from_bytes([2u8; 32]).unwrap();
        let user_key: PublicKey = PublicKey::from(&tmp4);
        let user_addr = AccountHash::from(&user_key);

        // Create plain user, will not receive access rights.
        let tmp5 = SecretKey::ed25519_from_bytes([3u8; 32]).unwrap();
        let unauth_key = PublicKey::from(&tmp5);
